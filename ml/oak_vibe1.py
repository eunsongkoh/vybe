# stage3_identifiers_wide_fixed.py
# Wide FOV + correct NN input + proper box mapping back to 640x360.
# Prints JSON once/sec. ESC to quit.
...
man.setKeepAspectRatio(True)         # letterbox (no distortion)
man.setFrameType(dai.ImgFrame.Type.BGR888p)  # RGB/BGR both fine for SSD blobs
...
if use_nn:
    ...
    man.out.link(nn.input)          # feed the 300x300 letterboxed image
    xnn = p.createXLinkOut(); xnn.setStreamName("nn"); nn.out.link(xnn.input)

with dai.Device(p) as dev:
    qprev = dev.getOutputQueue("preview", 4, False)      # 640x360 frames (display/flow)
    ...
    # Precompute letterbox mapping constants for 640x360 -> 300x300
    in_w, in_h = 640, 360
    nn_w, nn_h = 300, 300
    s = min(nn_w / in_w, nn_h / in_h)      # scale to fit inside 300x300
    ...
    def map_nn_box_to_preview(xmin, ymin, xmax, ymax):
        """
        Map normalized NN coords (relative to 300x300 letterboxed canvas)
        back to 640x360 preview coordinates.
        """
    ...
    while True:
        frame = qprev.get().getCvFrame()    # 640x360
        h, w  = frame.shape[:2]
        gray  = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

        # Update detections (non-blocking drain)
        if use_nn:
            while qnn.has():
        ...
        if prev_gray is None:
            ...
            cv2.imshow("OAK-1 Identifiers (Wide FOV) — ESC to quit", frame)
        ...
        # Mask from people boxes (or full frame)
        if boxes_640x360:
        ...
        energy = 0.0 if hi <= lo else float(np.clip((raw_intensity - lo)/(hi - lo), 0.0, 1.0))
        ...
        ema_vibe = vibe_raw if ema_vibe is None else (ALPHA_VIBE*vibe_raw + (1-ALPHA_VIBE)*ema_vibe)
        ...
        cv2.putText(frame,
            f"vibe={vibe:.2f}  energy={energy:.2f}  engage={engagement:.2f}  cons={consistency:.2f}  people={people}",
            (10,24), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,0), 2)
        if bounce_bpm is not None:
            cv2.putText(frame, f"bpm~{bounce_bpm:.0f}", (10,48),
